...
#include "DsRecoAnalysis4FitDsM.C"

ClassImp(DsRecoAnalysis4);
DsRecoAnalysis4::DsRecoAnalysis4(Bool_t MCorDATA,Bool_t TruthMatch,Int_t MatterOrAntiMatter,TString OutputDir):
  TagAnalysis(MCorDATA,TruthMatch,MatterOrAntiMatter,OutputDir)
{
  cout<<"Done Initializing the analysis"<<endl;
}

DsRecoAnalysis4::~DsRecoAnalysis4(){ 
}

Int_t DsRecoAnalysis4::ReduceNtuple(Bool_t RunInTestMode){
...
void DsRecoAnalysis4::FillDsRecoVars(){
  ...
  //cout<<TagIdx<<" "<<TagKIdx<<" "<<FragIdx<<" "<<DsStIdx<<endl;
  ...
  TVector3 DsStp3V(DsStp3[DsStIdx]*sin(acos(DsStcosth[DsStIdx]))*cos(DsStphi[DsStIdx]),
		   DsStp3[DsStIdx]*sin(acos(DsStcosth[DsStIdx]))*sin(DsStphi[DsStIdx]),
		   DsStp3[DsStIdx]*DsStcosth[DsStIdx]);
  TLorentzVector DsStp4(DsStp3V,DsStenergy[DsStIdx]);
  ...
  TLorentzVector Gamp4(Gamp3V,Gamp3V.Mag());
  TLorentzVector Gamp4CM=Gamp4;Gamp4CM.Boost(-eeVelVec);
  ...
  TLorentzVector Tagp4(Tagp3vec,Tagenergy[TagIdx]);
  ...
  //the Ds
  TLorentzVector Dsp4=DsStp4-Gamp4;  
  ...
  gamenergy=Gamp4.T();
  gamdsstangle=Gamp4.Vect().Unit()*DsStp4.Vect().Unit();
  ...
  fragexgame=sqrt(Evtexgampx[EvtIdx]*Evtexgampx[EvtIdx]
		  +Evtexgampy[EvtIdx]*Evtexgampy[EvtIdx]
		  +Evtexgampz[EvtIdx]*Evtexgampz[EvtIdx]);
...
Float_t DsRecoAnalysis4::ComputeGamAngle(){
...
Float_t DsRecoAnalysis4::ComputeKAngle(){
  //must be in CM frame
...
Float_t DsRecoAnalysis4::ComputeFragAngle(){
  //must be in CM frame
  TVector3 Tagp3vec(Tagp3CM[TagIdx]*sin(acos(TagcosthCM[TagIdx]))*cos(TagphiCM[TagIdx]),
		    Tagp3CM[TagIdx]*sin(acos(TagcosthCM[TagIdx]))*sin(TagphiCM[TagIdx]),
		    Tagp3CM[TagIdx]*TagcosthCM[TagIdx]);
...
Int_t DsRecoAnalysis4::SetDsRecoBranches(){
  if(_fChain==NULL){cout<<"No Chain"<<endl;return 0;} 
  
  _fChain->SetBranchAddress("nEvt",&nEvt);
  _fChain->SetBranchAddress("EvtnDaus",EvtnDaus);
  _fChain->SetBranchAddress("EvtChi2",EvtChi2);
  _fChain->SetBranchAddress("EvtnDof",EvtnDof);
  ...
  _fChain->SetBranchAddress("Evtgampx",Evtgampx);
  _fChain->SetBranchAddress("Evtgampy",Evtgampy);
  _fChain->SetBranchAddress("Evtgampz",Evtgampz);
  _fChain->SetBranchAddress("EvtDsStUncMass",EvtDsStUncMass);
  _fChain->SetBranchAddress("Evtexgampx",Evtexgampx);
  _fChain->SetBranchAddress("Evtexgampy",Evtexgampy);
  _fChain->SetBranchAddress("Evtexgampz",Evtexgampz);
  _fChain->SetBranchAddress("Evtexgamn",Evtexgamn);
...
Int_t DsRecoAnalysis4::MakeDsRecoReducedNtupleBranches(TTree* Ntuple){
...
Int_t DsRecoAnalysis4::SetDsRecoReducedNtupleBranches(){
