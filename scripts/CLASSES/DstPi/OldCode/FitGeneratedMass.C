...
void FitGeneratedMass(Int_t whichpeak,Int_t nfiles){
  //------------------------
  //choose the peak
  //-----------------------
  TString _OutPutDir="DstarCharged/DstarPi/MC/D2420/Kpi";
  Int_t LundId=10423;
  ...
  if(whichpeak==2){
    ...
    _OutPutDir="DstarCharged/DstarPi/MC/D2460/Kpi";
    LundId=425;
    meanmass=2.459;
    nwindows=100;
    L=2;
  ...
  TString filename=_OutPutDir+"/FitGeneratedMass.ps"; 
  ...
  JResiduals MassRes(&HMass,MassFit);     
  Canvas.Clear();
  ...
  char temp[100];
  sprintf(temp,"1*(-%.5f<x&&x<%.5f)",binwidth/2.,binwidth/2.);
  TF1 Flat("Flat",temp,-binwidth/2,binwidth/2);
  ...
  JResiduals MassNewRes(&HMassNew,MassNewFit);     
  ...
  sprintf(temp,"#chi^{2}/ndf=%.1f/%d",MassNewFit->GetChisquare(),MassNewFit->GetNDF());
  stats.AddText(temp);
  sprintf(temp,"M=%.5f+-%.5f",MassNewFit->GetParameter(1),MassNewFit->GetParError(1));
  stats.AddText(temp);
  sprintf(temp,"#Gamma=%.5f+-%.5f",MassNewFit->GetParameter(2),MassNewFit->GetParError(2));
  stats.AddText(temp);
  stats.Draw();
  Canvas.Print(filename);
  ...
  JResiduals MassRooRes(&HMassNew,FitHisto);     
  Canvas.Clear();
  ...
  sprintf(temp,"#chi^{2}/ndf=%.1f/%d",MassRooRes.GetChisquare(),HMassNew.GetXaxis()->GetNbins()-3);
  roostats.AddText(temp);
  sprintf(temp,"M=%.5f+-%.5f",mean.getVal(),mean.getError());
  roostats.AddText(temp);
  sprintf(temp,"#Gamma=%.5f+-%.5f",width.getVal(),width.getError());
  roostats.AddText(temp);
  roostats.Draw();
  Canvas.Print(filename);
