...
Int_t Dst0PiAnalysis::PlotDMVsPstarCosHel(){
  ...
  //
  //------------------------------
  TTree* ReducedNtupleCopy=ReducedNtuple->CopyTree("");
  ...
  SetHistoXYLabels(&HDMAll,"m(D*#pi)","GeV/c^{2}");  
  Canvas.Clear();
  HDMAll.Draw();
  Canvas.Print(filename);
  ...
  for(Int_t c=0;c<NCosHelBins;c++){    
    ...
    for(Int_t p=0;p<NPstarBins;p++){         
      ...
      SetHistoXYLabels(HDM[c][p],"m(D*#pi)","GeV/c^{2}");
      Canvas.Clear();
      HDM[c][p]->Draw("pe");
      Canvas.Print(filename);           
    }
...
//       HDM[c][p]->GetYaxis()->SetTitleOffset(1.1);
      HDM[c][p]->Draw("pe");  
    }
  Canvas.Print(filename);
  ...
  for(Int_t c=0;c<NCosHelBins;c++){    
    ...
    for(Int_t p=0;p<NPstarBins;p++){         
      ...
      ReducedNtupleCopy->Draw(TString("dstpideltam>>")+title,coscut+pcut);
      SetHistoXYLabels(HDMInclusive[c][p],"m(D*#pi)","GeV/c^{2}");
      Canvas.Clear();
...
//       HDMInclusive[c][p]->GetYaxis()->SetTitleOffset(1.1);
      HDMInclusive[c][p]->Draw("pe");  
    }
  Canvas.Print(filename);
