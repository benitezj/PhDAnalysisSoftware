...
#include "DsTolnuAnalysisFitDsStDeltaM.C"
...
Int_t DsTolnuAnalysis::ReduceNtuple(Bool_t RunInTestMode){
  ...
  while(_fChain->GetEntry(eventid,0)>0&&eventid<MaxNumberEvents){   
	...
	if(abs(mcLund[mcc])==myDsLund){
	  DsGenMCIdx=mcc;
	  if(abs(mcLund[dauIdx[mcc]])==myNuLund)NuGenMCIdx=dauIdx[mcc];
	  if(abs(mcLund[dauIdx[mcc]+1])==myNuLund)NuGenMCIdx=dauIdx[mcc]+1;	  
	}
      }
    }
...
void DsTolnuAnalysis::FillDsTolnuVars(){
  ...
  nup=Nup3CM[NuIdx];
  nupx=Nup3CM[NuIdx]*sin(acos(NucosthCM[NuIdx]))*cos(NuphiCM[NuIdx]);
  nupy=Nup3CM[NuIdx]*sin(acos(NucosthCM[NuIdx]))*sin(NuphiCM[NuIdx]);
  nupz=Nup3CM[NuIdx]*NucosthCM[NuIdx];  
  if(NuGenMCIdx>0){
    nupxres=nupx-mcp3CM[NuGenMCIdx]*sin(acos(mccosthCM[NuGenMCIdx]))*cos(mcphiCM[NuGenMCIdx]);
    nupyres=nupy-mcp3CM[NuGenMCIdx]*sin(acos(mccosthCM[NuGenMCIdx]))*sin(mcphiCM[NuGenMCIdx]);
    nupzres=nupz-mcp3CM[NuGenMCIdx]*mccosthCM[NuGenMCIdx];
  }else{ nupxres=100;  nupyres=100;  nupzres=100;}
...
Int_t DsTolnuAnalysis::SetDsTolnuBranches(){
  ...
  _fChain->SetBranchAddress("Dsd1Lund",Dsd1Lund);
  _fChain->SetBranchAddress("Dsd1Idx",Dsd1Idx);
  _fChain->SetBranchAddress("Dsd2Lund",Dsd2Lund);
  _fChain->SetBranchAddress("Dsd2Idx",Dsd2Idx);
  ...
  _fChain->SetBranchAddress("nNu",&nNu);
  _fChain->SetBranchAddress("Nup3",Nup3);
  _fChain->SetBranchAddress("Nucosth",Nucosth);
  _fChain->SetBranchAddress("Nuphi",Nuphi);
  _fChain->SetBranchAddress("Nuenergy",Nuenergy);
  _fChain->SetBranchAddress("Nup3CM",Nup3CM);
  _fChain->SetBranchAddress("NucosthCM",NucosthCM);
  _fChain->SetBranchAddress("NuphiCM",NuphiCM);
  _fChain->SetBranchAddress("NuenergyCM",NuenergyCM);
  _fChain->SetBranchAddress("NuLund",NuLund); 
...
Int_t DsTolnuAnalysis::MakeDsTolnuReducedNtupleBranches(TTree* Ntuple){
  ...
  Ntuple->Branch("nup",&nup,"nup/F"); 
  Ntuple->Branch("nupx",&nupx,"nupx/F"); 
  Ntuple->Branch("nupy",&nupy,"nupy/F"); 
  Ntuple->Branch("nupz",&nupz,"nupz/F"); 
  Ntuple->Branch("nupxres",&nupxres,"nupxres/F"); 
  Ntuple->Branch("nupyres",&nupyres,"nupyres/F"); 
  Ntuple->Branch("nupzres",&nupzres,"nupzres/F"); 
