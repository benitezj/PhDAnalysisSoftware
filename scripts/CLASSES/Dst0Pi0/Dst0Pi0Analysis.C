...
Int_t Dst0Pi0Analysis::ReduceNtuple(Bool_t RunInTestMode){
  ...
  Int_t RemovedCands=1;
  Int_t InitialNCands=1;
  Int_t RemovedEvents=1;
  ...
  while(_fChain->GetEntry(eventid,0)>0&&eventid<MaxNumberEvents){   
    ...
    for(XIdx=0;XIdx<nX;XIdx++){
      ...
      if(DstarIdx>=MAXNCANDS){RemovedCands++;continue;}
      Pi0Idx=Xd2Idx[XIdx];
      if(Pi0Idx>=MAXNCANDS){RemovedCands++;continue;}
      Gam1Idx=Pi0d1Idx[Pi0Idx];
      if(Gam1Idx>=MAXNCANDS){RemovedCands++;continue;}
      Gam2Idx=Pi0d2Idx[Pi0Idx]; 
      if(Gam2Idx>=MAXNCANDS){RemovedCands++;continue;}
      D0Idx=Dstard1Idx[DstarIdx];
      if(D0Idx>=MAXNCANDS){RemovedCands++;continue;}
      SlowPiIdx=Dstard2Idx[DstarIdx];
      if(SlowPiIdx>=MAXNCANDS){RemovedCands++;continue;}
      SlowGam1Idx=Pi0d1Idx[SlowPiIdx];
      if(SlowGam1Idx>=MAXNCANDS){RemovedCands++;continue;}
      SlowGam2Idx=Pi0d2Idx[SlowPiIdx]; 
      if(SlowGam2Idx>=MAXNCANDS){RemovedCands++;continue;}
      KIdx=D0d1Idx[D0Idx];
      if(KIdx>=MAXNCANDS){RemovedCands++;continue;}
      KtrkIdx=KTrkIdx[KIdx];
      if(KtrkIdx>=MAXNCANDS){RemovedCands++;continue;}
      PiIdx=D0d2Idx[D0Idx];
      if(PiIdx>=MAXNCANDS){RemovedCands++;continue;}
      PitrkIdx=PiTrkIdx[PiIdx];
      if(PitrkIdx>=MAXNCANDS){RemovedCands++;continue;}
      D0Pi2Idx=D0d3Idx[D0Idx];
      if(D0Pi2Idx>=MAXNCANDS){RemovedCands++;continue;}
      D0Pi2trkIdx=PiTrkIdx[D0Pi2Idx];
      if(D0Pi2trkIdx>=MAXNCANDS){RemovedCands++;continue;}
      D0Pi3Idx=D0d4Idx[D0Idx];                        
      if(D0Pi3Idx>=MAXNCANDS){RemovedCands++;continue;}      
      D0Pi3trkIdx=PiTrkIdx[D0Pi3Idx];
      if(D0Pi3trkIdx>=MAXNCANDS){RemovedCands++;continue;}
      
      //cout<<nX<<" "<<XIdx<<" "<<DstarIdx<<" "<<Pi0Idx<<" "<<Gam1Idx<<" "<<Gam2Idx<<" "<<SlowPiIdx<<" "<<SlowGam1Idx<<" "<<SlowGam2Idx<<endl;
	...
	dstpilogvtxprob=log10(TMath::Prob(XChi2[XIdx],XnDof[XIdx]));
      ...
      pi0p3=Pi0p3[Pi0Idx];
      pi0energy=sqrt(pi0mass*pi0mass+pi0p3*pi0p3);     
      ...
      slowpihelicity=ComputeSlowPiHelicityAngle();      
  ...
  cout<<"Total candidates removed "<<RemovedCands<<" of "<<InitialNCands<<" ( "<<floor(100*RemovedCands/InitialNCands + .5)<<"%)"<<endl;
  //This is very strange if I include this line the code breaks????
  //cout<<"Total candidates removed "<<RemovedCands<<" of "<< InitialNCands <<endl;
  cout<<"Total events removed "<<RemovedEvents<<" of "<<eventid<<" ( "<<(int)(100*RemovedEvents/eventid + .5)<<"%)"<<endl;
...
Float_t Dst0Pi0Analysis::ComputePiAngle(){
  TVector3 Xp3vec(Xp3CM[XIdx]*sin(acos(XcosthCM[XIdx]))*cos(XphiCM[XIdx]),
		  Xp3CM[XIdx]*sin(acos(XcosthCM[XIdx]))*sin(XphiCM[XIdx]),
		  Xp3CM[XIdx]*XcosthCM[XIdx]);
...
Float_t Dst0Pi0Analysis::ComputePiHelicityAngle(){
  TVector3 Dstarp3vec(Dstarp3CM[DstarIdx]*sin(acos(DstarcosthCM[DstarIdx]))*cos(DstarphiCM[DstarIdx]),
		  Dstarp3CM[DstarIdx]*sin(acos(DstarcosthCM[DstarIdx]))*sin(DstarphiCM[DstarIdx]),
		  Dstarp3CM[DstarIdx]*DstarcosthCM[DstarIdx]);
...
Float_t Dst0Pi0Analysis::ComputePiDstarAngle(){
...
Float_t Dst0Pi0Analysis::ComputeSlowPiHelicityAngle(){
  ...
  TVector3 SlowPip3vec(Pi0p3CM[SlowPiIdx]*sin(acos(Pi0costhCM[SlowPiIdx]))*cos(Pi0phiCM[SlowPiIdx]),
		    Pi0p3CM[SlowPiIdx]*sin(acos(Pi0costhCM[SlowPiIdx]))*sin(Pi0phiCM[SlowPiIdx]),
		    Pi0p3CM[SlowPiIdx]*Pi0costhCM[SlowPiIdx]); 
  
  TLorentzVector SlowPip4(SlowPip3vec,sqrt(SlowPip3vec*SlowPip3vec+Pi0PDGMass*Pi0PDGMass));
  
  //boost the dstar momentum into rest frame of D0
  SlowPip4.Boost(-D0Velvec);//watch the minus sign
  TVector3 SlowPiBoostedp3vec=SlowPip4.Vect();

  //dot product of boosted dstar direction with D0 direction 
  return SlowPiBoostedp3vec.Unit()*D0p3vec.Unit();
...
Int_t Dst0Pi0Analysis::MakeReducedNtupleBranches(){
  ...
  ReducedNtuple->Branch("pi0p3",&pi0p3,"pi0p3/F");
  ...
  ReducedNtuple->Branch("slowpimass",&slowpimass,"slowpimass/F");
  ReducedNtuple->Branch("slowpip3",&slowpip3,"slowpip3/F");
  ReducedNtuple->Branch("slowpienergy",&slowpienergy,"slowpienergy/F");
  ReducedNtuple->Branch("slowpimctrue",&slowpimctrue,"slowpimctrue/I");
  ReducedNtuple->Branch("slowpihelicity",&slowpihelicity,"slowpihelicity/I");
...
Int_t Dst0Pi0Analysis::SetReducedNtupleBranches(){
  ...
  ReducedNtuple->SetBranchAddress("pi0p3",&pi0p3);
  ...
  ReducedNtuple->SetBranchAddress("slowpimass",&slowpimass);
  ReducedNtuple->SetBranchAddress("slowpip3",&slowpip3);
  ReducedNtuple->SetBranchAddress("slowpienergy",&slowpienergy);
  ReducedNtuple->SetBranchAddress("slowpimctrue",&slowpimctrue);
  ReducedNtuple->SetBranchAddress("slowpihelicity",&slowpihelicity);
