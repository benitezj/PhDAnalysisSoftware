...
BToDstGamAnalysis* analyzeBToDstGam(Int_t WhichDATASet=-1,Bool_t ReduceNtuple=0){
  ...
  Int_t TruthMatch=0;
  Bool_t MCorDATA=0;
  TString myntupledir="DstarCharged/BToDstarGam";  
  BToDstGamAnalysis* a=NULL;
  ...
  if(WhichDATASet==1){
    ...
    TruthMatch=0;
    MCorDATA=1;
    a=new BToDstGamAnalysis(MCorDATA,TruthMatch,MatterOrAntiMatter,OutputDir);
  ...
  if(WhichDATASet==12){
    ...
    if(ReduceNtuple){
      ...
      TreeChain(chain,1,53,myntupledir+"/DATARun6On");
      a->SetChain(chain);      
      a->SetReduceCuts(1,0,1,0);
    }
   
  }
  if(WhichDATASet==13){
    OutputDir=myntupledir+"/BThrust"; 
    MatterOrAntiMatter=0;   
    TruthMatch=0;
    MCorDATA=1;
    a=new BToDstGamAnalysis(MCorDATA,TruthMatch,MatterOrAntiMatter,OutputDir);
